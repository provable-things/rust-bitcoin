...
impl BlockHeader {
    ...
    pub fn validate_pow(&self, required_target: &Uint256) -> Result<(), util::Error> {
        ...
        let block_header_vector = &serialize(self);
...
impl BitcoinHash for BlockHeader {
    fn bitcoin_hash(&self) -> sha256d::Hash {
        use consensus::encode::serialize;
        // serialize encode and object into a vector
...
mod tests {
    ...
    fn test_scrypt() {
        ...
        let version: u32 = 536870912;
        let merkle_root = sha256d::Hash::from_str(
            "3b776a5cc5e78bba1356414ea6b145e94c45d9ec2274a30acd8e89a28b392dc2",
        )
        .unwrap();
        let prev_blockhash = sha256d::Hash::from_str(
            "3dadd3dd482899a545b7e0548016db256e3acdb7bae1831e94b710d17a06e15d",
        )
        .unwrap();
        let litecoin_header =
            BlockHeader::new(time, bits, nonce, version, merkle_root, prev_blockhash);
    ...
    fn should_create_new_block_header_from_constituent_parts() {
        ...
        .unwrap();
        let block = BlockHeader::new(time, bits, nonce, version, merkle_root, prev_blockhash);
